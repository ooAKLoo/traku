    ...
    private func startExport() {
        ...
        Task {
            do {
                ...
                await updateProgress(0.3, "获取空间和分类数据...")
                let spaces = databaseManager.getAllSpaces()
                let categories = spaces.flatMap { space in
                    databaseManager.getCategories(for: space.id)
                }
                ...
                let exportData = ExportData(
                    ...
                    spaces: spaces.map { ExportSpace(from: $0) },
                    categories: categories.map { ExportCategory(from: $0) },
...
struct ExportCategory: Codable {
    ...
    init(from category: Category) {
        ...
        self.spaceId = category.spaceId.uuidString
    }
}
